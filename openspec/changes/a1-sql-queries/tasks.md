...
- [x] 2.1 Crear `worky_engine/sql/analysis/a1_00_last_touch.sql` con la vista `mart_last_touch` a grano deal, simétrica a `mart_first_touch` pero con el orden y el empate invertidos. ~40 líneas. Decisión D7.
- [x] 2.2 Crear `worky_engine/sql/analysis/a1_03_cohort_retention.sql` con cohortes por mes de `signup_date`, formato largo (una fila por cohorte y k), `dataset_end` derivado de `dataset_asof` y `cell_status` en `censored` para celdas sin k meses cumplidos. ~70 líneas. Requirement A1.3, dos escenarios. Decisiones D6, D8, D9, D10.
- [x] 2.3 Crear `worky_engine/sql/analysis/a1_04_attribution.sql` con atribución a grano deal por primer y último touch, conversión a `closedwon`, tasa por canal en ambos modelos y `channel_rank`. ~80 líneas. Requirement A1.4, dos escenarios. Decisión D6, D7.
- [x] 2.4 Modificar `worky_engine/analysis/runner.py`: agregar `a1_00_last_touch.sql`, `a1_03_cohort_retention.sql` y `a1_04_attribution.sql` a `ANALYSIS_FILES`/`ANALYSIS_OUTPUTS`, en ese orden porque `a1_04` consume la vista de `a1_00`. ~10 líneas. Decisión D6.
- [x] 2.5 Modificar `worky_engine/quality/analysis_contracts.py`: agregar `assert_a1_03_pct_within_range`, `assert_a1_03_monotone_non_increasing`, `assert_a1_03_retained_within_cohort_size`, `assert_a1_04_rate_within_unit` y `assert_a1_04_models_cover_same_deals`. ~45 líneas. Decisiones D12, D14.
- [x] 2.6 Modificar `tests/test_analysis_rules.py`: agregar al fixture dos touches con la misma fecha (empate por `touch_id` en los dos extremos), un deal creado antes del primer touch de su empresa, una cohorte reciente que fuerza celdas censuradas y el caso frontera de una empresa que hace churn exactamente en el mes k. Prueba: `python -m pytest -q -m "not dataset" tests/test_analysis_rules.py`. ~120 líneas. Requirement A1.3 (celda censurada, retención por cohorte y k), A1.4 (dos canales por deal, tasas lado a lado y bandera de cambio de ganador).
- [x] 2.7 Modificar `tests/test_analysis_dataset_numbers.py`, marca `dataset`: agregar que los deals atribuidos son iguales en los dos modelos (997 menos los 35 huérfanos de A1.5) y que A1.3 nunca sube de un k al siguiente dentro de una misma cohorte sobre el dataset real. ~40 líneas. Decisión D12.
- [x] 2.8 Modificar `tests/test_analysis_idempotency.py`, marca `dataset`: extender la comparación byte a byte a `a1_03_cohort_retention.csv` y `a1_04_attribution.csv`, y mantener la verificación del hash de los ocho archivos de `outputs/` (A0, read-only). Decisión D15. Nota: el diseño no listó esta modificación en su tabla de líneas del PR 2; se agrega aquí para no dejar sin cobertura las salidas nuevas hasta el cierre del PR 3 (ver brecha en el resumen de esta fase).
- [x] 2.9 Cerrar PR 2: correr `python -m pytest -q -m "not dataset"`, correr `python -m pytest -q -m dataset tests/test_analysis_dataset_numbers.py tests/test_analysis_idempotency.py`, correr `analyze` dos veces y confirmar byte-identidad de las cinco salidas acumuladas, confirmar `git status --short outputs` (A0, read-only) sin cambios, y generar y commitear `outputs/analysis/a1_03_cohort_retention.csv` y `a1_04_attribution.csv`.
...
## Notas de la aplicación del PR 1

- **Cifra medida distinta de la citada en ADR-004/diseño**: sobre el dataset real, A1.2 entrega las 89 filas esperadas, pero `windows_overlap` da 33 cuentas, no las 30 que cita el ADR-004 y la sección 2.2 del diseño. La consulta implementa la regla tal cual ("`usage_months < 6`", decisión D6), y el propio diseño anticipa este tipo de discrepancia en su tabla de riesgos residuales ("el reporte publica el número medido en lugar de que la prueba se ajuste"). `tests/test_analysis_dataset_numbers.py` fija 33 con un comentario que documenta el desglose (3 cuentas con 0 meses, 1 con 2, 6 con 3, 12 con 4, 11 con 5). Pendiente: una adenda al ADR-004 que corrija la cifra citada de 30 a 33.
