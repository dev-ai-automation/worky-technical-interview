...
| account_id | VARCHAR | sí por contrato, poblado en las 650 filas con el dataset real (596 por T0 y 54 por T2) | `ACC-2027` |
...
Un valor del CRM nunca se sobrescribe. Cuando una empresa sin MRR conserva más de un monto distinto después de la normalización a mensual, no se imputa: la fila queda en `unresolved` y se registra `imputation_ambiguous`. En este dataset ese caso sale en cero, y la guarda existe para que un dato nuevo no se resuelva por accidente.
...
| Un deal anual sin ninguna pareja mensual en su empresa no se detecta por la razón 12 y entra como si fuera mensual | Media | Límite conocido y declarado: la regla de la sección 4.3 solo puede reconocer la unidad cuando la empresa tiene los dos montos. En este dataset toda combinación medida cae en 1.0 o en 12.0, así que no hay caso suelto detectable; el reporte declara la unidad y una fila que resulte inflada por 12 se ve como un valor atípico contra su segmento |
