...
4. `identity_overrides` guarda la decisión de una persona sobre un registro que cayó en revisión manual (nivel M), y el proceso de resolución la aplica justo después de la cascada de A0 y antes de escribir el crosswalk y la auditoría, sin modificar los archivos existentes de ese módulo. Es la pieza que faltaba para que ese trabajo humano no se pierda en cada rebuild.
...
| Dónde viven las llaves de reconciliación (pregunta A4 del caso) | Las dos cosas a la vez: `identity_crosswalk` sigue siendo la tabla de mapeo persistente, y la cascada de A0 sigue siendo el proceso que la resuelve, aplicando `identity_overrides` después de la cascada y antes de escribir, y, con presupuesto disponible, una marca de agua que solo procese `source_id` no vistos | El caso pregunta si es tabla o proceso como si fueran alternativas; ya son las dos cosas hoy, y elegir solo una perdería la mitad de la respuesta real |
...
| `identity_overrides` toca un módulo que A0 ya cerró y archivó | Es una extensión aditiva: ningún archivo existente de `identity_resolution` cambia, la tabla y sus pruebas viven en archivos nuevos, y el proceso de resolución la aplica después de la cascada y antes de escribir el crosswalk y la auditoría |
